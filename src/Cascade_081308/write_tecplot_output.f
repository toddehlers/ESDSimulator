...
c open files - hard-coded for now (11/06)
      open (47,file='output/IceCascade/topo_tec_'//timenice//'.dat',
     &      status='unknown')
c Write header(s)
      write(47,'(A34)') 'TITLE = "Cascade model topography"'
      write(47,'(A200)') 'VARIABLES = "x" "y" "z" "node" "Precipitation"
     & "Fluvial Erosion Rate" "Diffusion Erosion Rate"
     & "Landslide Erosion Rate" "Total Erosion Rate" "Catchment Color"
     & "Catchment Number"'
      write(47,'(A18)') 'ZONE T = "Cascade"'
      write(47,900) 'n=',nnode,', e=',nt,', et=triangle, f=fepoint'

c Write topography
      do i=1,nnode
      write(47,901) x(i),y(i),h(i)/1000,i,prec(i),
