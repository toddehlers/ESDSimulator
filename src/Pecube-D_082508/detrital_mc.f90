...
! Variable declaration
      ...
      real*8,dimension(:,:),allocatable :: pmc
      integer,dimension(:),allocatable :: kuiper_res
      real*8 :: d,prob,pagemu,pagemed,pagesd,pdfvsc,mc_iterf,jf,kpct
      real*8 :: dx,osum,randflt,agenow,psummc,pi,peratemin,peratescl
      integer :: olc,onum,oamin,oamax,h,i,j,k,l,mc_iter,basnum
      integer :: plc,plcsc,cnt,rint,paminmc,pamaxmc,pnummc,cnt2,cnt3,hm,hm2,cnt4
      character :: buffer*8,obasin*300,pbasin*8,jc*3,hc*5,run*100
      real*8 :: oage(olc),oageu(olc),age_temp,oageu_percent(olc)
      real*8 :: page(plc),perate(plc),pageu(plc),peratesc(plc),page2(plc)
      real*8 :: x_basin,y_basin
      character :: x_basin_char*20,y_basin_char*20
      integer :: num_xchars,num_ychars,nrun

      write(6,*)
      write(6,'(A27)') 'Performing Monte Carlo runs'
      write(6,'(A20)') 'This may take awhile'
      write(6,'(A39)') 'Every 500th iteration will be announced'
      write(6,*)
...
!       open(10,file='basin_summary_info.txt',status='old')
      call system('if [ ! -d '//run(1:nrun)//'/mc_pdf_output ]; then mkdir '//run(1:nrun)//'/mc_pdf_output; fi')
      call system('if [ ! -d '//run(1:nrun)//'/mc_pdf_output/Basin_X_'//x_basin_char(num_xchars:20)//'_Y_'//y_basin_char(num_ychars:20)//' ]; then mkdir '//run(1:nrun)//'/mc_pdf_output/Basin_X'//x_basin_char(num_xchars:20)//'_Y'//y_basin_char(num_ychars:20)//'; fi')
      call system('if [ ! -d '//run(1:nrun)//'/mc_pdf_output/Basin_X_'//x_basin_char(num_xchars:20)//'_Y_'//y_basin_char(num_ychars:20)//'/pass_mc_age_PDF ]; then mkdir '//run(1:nrun)//'/mc_pdf_output/Basin_X'//x_basin_char(num_xchars:20)//'_Y'//y_basin_char(num_ychars:20)//'/pass_mc_age_PDF; fi')
      call system('if [ ! -d '//run(1:nrun)//'/mc_pdf_output/Basin_X_'//x_basin_char(num_xchars:20)//'_Y_'//y_basin_char(num_ychars:20)//'/mc_age_PDF ]; then mkdir '//run(1:nrun)//'/mc_pdf_output/Basin_X'//x_basin_char(num_xchars:20)//'_Y'//y_basin_char(num_ychars:20)//'/mc_age_PDF; fi')
...
!         if (i.eq.5 .or. i.eq.15 .or. i.eq.16) then
      ...
      do i=1,olc
        sumsd = sumsd + (oageu_percent(i)-pagemu)**2.
      enddo
...
! Generate erosion-rate-scaled model age distribution
          plcsc=sum(peratesc)                                                     ! Determine number of ages in scaled model age distribution
...
!!!!
          ...
          do j=1,mc_iter                                                          ! Model age distribution; This loop runs mc_iter times (usually ~10000)
            ...
            if (j.eq.1 .or. mod(j,500).eq.0) write(6,'(A10,i5)') 'Iteration ',j
...
! Write output files
          ...
          write(21,'(a25,f10.2)') obasin,kpct                                   ! Write out the summary percent of models that passed the Kuiper test
