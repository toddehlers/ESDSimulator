...
!       dwhipp - 02/07
	...
	real*4 edot(nsurf),bg_edot(nsurf),topo_edot(nsurf)
	real*8 xlonmin,xlonmax,xmin,xmax,xlatmin,xlatmax,xxx,yyy,ymin,ymax
        character run*100,c3*3
        integer nrunnx,ny,nelemsurf,counter

        open (68,file=run(1:nrun)//'/erates_tec'//c3//'.dat',status='unknown')
...
!	Writes Tecplot header in Ages_tec.dat file
        ...
        write (68,'(a126)') 'VARIABLES = "x (km)" "y (km)" "z (km)" "Total Erosion rate &
                      (mm/yr)" "Background Erosion Rate (mm/yr)" "Relief Change &
                      (mm/yr)"'
