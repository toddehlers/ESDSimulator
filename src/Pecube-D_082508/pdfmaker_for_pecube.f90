...
subroutine pdfmaker_for_pecube (age,number,flag,t,c,run,edot,x_basin_char,y_basin_char,&
  ...
  real*8 dx,pi,i,amount,start,finish,avg_edot
...
!   print *, 'sum:',amount
    ...
    if (c .ne. -1) then
      ...
      do k=1,counter
        write (106,'(2f10.2)') n(k),normPDF(k)
      enddo
      close (106)

    else

      call system('if [ ! -d '//run(1:nrun)//'/pdf_pecube ]; then mkdir '//run(1:nrun)//'/pdf_pecube; fi')        ! Checks if the pdf_data folder exists and creates it if it doesn't
      ...
      do k=1,counter
        write (204,'(2f10.2)') n(k),normPDF(k)
      enddo
      close (204)
