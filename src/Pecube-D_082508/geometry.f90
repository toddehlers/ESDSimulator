      subroutine geometry (xin,yin,zin,vxp,vyp,vzp,time,xmin,xmax,ymin,&
                          ymax,zl,x1f,y1f,x2f,y2f,def,dif,geoflag,&
                          theta,phi,mft_ratein,mbt_ratein,mct_ratein,&
                          stf_ratein,Peclet,Peclet2)
...
! For given values of xp,yp,zp and time this routine should return a
! velocity field in the form of a triplet vxp,vyp,vzp

! The size of the box is given in terms of xmin,xmax, ymin, ymax and zl
...
! (it is not the depth but the height of the box)
      ...
      real*8 mbt_ratein,mft_dipin,mbt_dipin,mct_dipin,vin,stf_dipin,xx
      real*8 yy,x,y,z,xin,yin,zin,mht1_dipin,mht2_dipin,mht3_dipin
      real*8 mht4_dipin,mfts,mfti,mbts,mbti,mcts,mcti,stfs,stfi,mht1s
      real*8 mht1i,mht2s,mht2i,mht3s,mht3i,mht4s,mht4i,zl,mht4dip,kb0s
      ...
      real*8 mht3dip,vergin,ut_ratein,mft_rate,mbt_rate,mct_rate
      real*8 stf_rate,ut_rate,pie,halfpi,mftdip,mctdip,stf_dip,verge
      real*8 mft_ratein,mct_ratein,stf_ratein
      real*8 axis,azimuth,lambda,lambda_corr,k,dx,dy,phi1,phi2,h,d
      real*8 a1,a2,dist,vz,Peclet,Peclet2,veloin,veloout,amed,adif
! reset velocities
...
! Angle phi is the angle relative to the xy plane
      else if (geoflag .eq.2) then

        rad_theta=theta*(3.141592654/180)
        rad_phi=phi*(3.141592654/180)
...
! Changed vyp equation to replace vy with vx
      ...
      ! New Nepal model geometry using rotated grid - dwhipp (08/07)
        ...
        x = xin				! Rename xin
        y = yin				! Rename yin
        z = zin-zl			! Shift z-points down into elevations ASL (km)
	...
	kb7bi=-848.62			! Kink band plane 7b z-intercept - From Avouac cross-section (km)
        ...
        !***
	vin = vin + stf_ratein						! Account for extension on stf in keeping constant convergence rate
	ut_ratein = (vin - mft_ratein - mbt_ratein - mct_ratein)	! Underthrusting rate is equal to that not accounted for by the MFT and MCT
	mftrate=-mft_ratein						! Rename fault convergence rates (for simplicity)
	mbtrate=-mbt_ratein						! Rename fault convergence rates (for simplicity)
	mctrate=-mct_ratein						! Rename fault convergence rates (for simplicity)
	stfrate=stf_ratein						! Rename fault convergence rates (for simplicity)
	...
	verge = (pie/180.0)*vergin					! convert from deg to rad
        ...
        if (mft_ratein.eq.0. .and. mbt_ratein.eq.0. .and. mct_ratein.eq.0.) then
        ...
        else if (mft_ratein.eq.0. .and. mbt_ratein.eq.0.) then
          if (x.le.stf_seg) then                                                ! Point is west of STF tear
            if (y.le.(z-kb45i)/kb45s) then                                      ! If true, point is south of KB4.5
              ...
              if (z.gt.stfas*y+stfai) then                                      ! Point is in STFa hanging wall
                vyp=(mctrate+stfrate)
                if (stf_ratein.eq.0.) then                                      ! Ignore STF if inactive
            ...
            if (y.le.(z-kb45i)/kb45s) then                                      ! If true, point is south of KB4.5
              ...
              if (z.gt.stfbs*y+stfbi) then                                      ! Point is in STFb hanging wall
                ...
                vyp=utrate
                vzp=utrate*tan(mht4dip)
                if (stf_ratein.ne.0.) vzp=vzp+vup_gh                            ! Underplate if needed (to balance erosional removal)
              endif
            else                                                                ! Point is north of KB7b
              if (z.gt.mht4s*y+mht4i) then                                      ! Point is in MHT4 hanging wall
                vyp=(mctrate+stfrate)
          ...
          if (y.le.(z-kb15i)/kb15s) then                                        ! Point is south of KB1.5
            ...
            if (y.le.(z-kb5i)/kb5s) then                                        ! Point is south of KB5
              if (z.gt.stfas*y+stfai) then                                      ! Point is in STFa hanging wall
                ...
                if (stf_ratein.eq.0. .and. mct_ratein.eq.0.) then               ! Ignore STF and MCT if inactive
                  vzp=(mbtrate+mctrate+stfrate)*tan(mht3dip)
                else if (stf_ratein.eq.0.) then                                 ! Ignore STF if inactive
                ...
                vyp=(mbtrate+mctrate)
                if (mct_ratein.eq.0.) then                                      ! Ignore MCT if inactive
              ...
              if (z.gt.stfas*y+stfai) then                                      ! Point is in STFa hanging wall
                vyp=(mbtrate+mctrate+stfrate)
                if (stf_ratein.eq.0. .and. mct_ratein.eq.0.) then               ! Ignore MCT and STF if inactive
        ...
        ! MFT active (perhaps w/ MBT and MCT)
          ...
          if (y.le.(z-kb0i)/kb0s) then                                          ! If true, point is in front of thrust belt
            ...
            if (z.gt.mbts*y+mbti) then                                          ! Point in above MBT
              vyp=(mftrate+mbtrate)
              if (mbt_ratein.eq.0.) then                                        ! Ignore MBT if inactive
            ...
            vyp=(mftrate+mbtrate)
            if (mbt_ratein.eq.0.) then                                          ! Ignore MBT if inactive (Move parallel to MHT2)
...
! Edited by S. Olen, 6/18/10.
	...
	!  Set lateral velocities to zero
		...
		axis=((y2f-y1f)/(x2f-x1f))*(xin-x1f) + y1f
		...
		vzp=(dif-def)*(cos(k*d_corr)+1)/2+def
...
!
	else if (geoflag .eq. 6) then
		x1=x1f   !Coordinates of the ellipse's two foci
		y1=y1f
		x2=x2f
		y2=y2f

		a1 = def  !Inner and outer semi major axis
		a2 = dif
		...
		if(Peclet .eq. 0) then            !Calculate background velocity (veloout) as a fraction of inner velocity (veloin)
...
!
	else if (geoflag .eq. 7) then
		x1=x1f   !Coordinates of the ellipse's two foci
		y1=y1f
		x2=x2f
		y2=y2f

		amed = dif + def  !Sum of inner and outer semi major axis
		adif = (dif-def)/3.d0 !Difference of inner and outer semi major axis, division factor determines slope
...
! Final statement if the geometry value specified is not between 1 and 7	
        else
          write (6,*) 'Geometry flag value: ',geoflag
          print *, 'Geometry flag value is not between 1-7, please correct problem and start again'
          stop
        endif
