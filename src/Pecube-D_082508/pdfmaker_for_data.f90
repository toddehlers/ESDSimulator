...
subroutine pdfmaker_for_data (age,error,number,age_type,run,xoutlet,youtlet,nrun)
  ...
  call system('if [ ! -d '//run(1:nrun)//'/pdf_data ]; then mkdir '//run(1:nrun)//'/pdf_data; fi')	! Checks if the pdf_data folder exists in the run output directory and creates it if it does not exist
