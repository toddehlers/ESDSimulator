...
subroutine catchments_output (ftag,fteg,ftmg,ftzg,nstep,nsurf,run,xp,yp,det_calc,edot,nrun,&
  ...
  integer md,ncp,nstep,nsurf,nnode,cstore,number
  integer,dimension(:),allocatable :: catch,catch_count,iwk
  real ( kind = 8 ),dimension(:),allocatable :: x,y,z,eros_rate,wk,new_edot
  ...
  real ( kind = 4 ) ftag(nstep,nsurf),ftmg(nstep,nsurf),ftzg(nstep,nsurf)
  real ( kind = 4 ) fteg(nstep,nsurf,8),edot(nstep,nsurf)
  ...
  integer age_flags(11),num_topo_files
  ...
  do i=1,nstep
    ...
    allocate (new_ages(11,nnode),new_edot(nnode))
    allocate (iwk(max(31,27+ncp)*nsurf+nnode),wk(8*nsurf))
    ...
    age = ftag(i,:)
    call idbvip (md,ncp,nsurf,xp(i,:),yp(i,:),age,nnode,x,y,new_ages(9,:),iwk,wk)

    age = ftmg(i,:)
    call idbvip (md,ncp,nsurf,xp(i,:),yp(i,:),age,nnode,x,y,new_ages(10,:),iwk,wk)

    age = ftzg(i,:)
    call idbvip (md,ncp,nsurf,xp(i,:),yp(i,:),age,nnode,x,y,new_ages(11,:),iwk,wk)

    do p=1,8
      age = fteg(i,:,p)
      call idbvip (md,ncp,nsurf,xp(i,:),yp(i,:),age,nnode,x,y,new_ages(p,:),iwk,wk)
    enddo

    ! This interpolation is for the erosion rates. Used in scaling the PDF later.
    age = edot(i,:)
    call idbvip (md,ncp,nsurf,xp(i,:),yp(i,:),age,nnode,x,y,new_edot,iwk,wk)

    call system('if [ ! -d '//run(1:nrun)//'/pdf_cascade ]; then mkdir '//run(1:nrun)//'/pdf_cascade; fi')     ! Checks if the pdf_pecube folder exists in the associated run output directory; if not, then it creates it
    ...
    do k=1,max_catch                                                    ! Iterates through all catchments
      if (catch_count(k).ge.node_thresh) then                           ! If the catchment is not empty
        ...
        if (det_calc .eq. '1') then                                     ! If a PDF is wanted
          ...
          allocate (pdf_ages_all(11,catch_count(k)),pdf_edot_all(11,catch_count(k)))
          pdf_ages_all=0.
          pdf_edot_all=0.
          do p=1,11                                                     ! Iterates through all types of ages
...
!               call pdfmaker_for_pecube (pdf_ages,catch_count(k),p,i,k,run,pdf_edot,'','',-1,-1,nrun)
          ...
          call pdfmaker_for_cascade (pdf_ages_all,catch_count(k),i,k,run,pdf_edot_all,'','',-1,-1,&
        ...
        call system('if [ ! -d '//run(1:nrun)//'/catchments ]; then mkdir '//run(1:nrun)//'/catchments; fi')
