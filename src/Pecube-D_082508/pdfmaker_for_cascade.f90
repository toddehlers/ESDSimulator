...
subroutine pdfmaker_for_cascade (age,number,t,c,run,edot,x_basin_char,y_basin_char,&
                                num_xchars,num_ychars,nrun,age_flags,header_info)
  ...
  real*8 age(11,number),error(11,number),per_error(11),edot(11,number)
  character timestep*4,catchment*4,run*100,age_flag*2
  character x_basin_char*100,y_basin_char*100
  integer num_xchars,num_ychars,nrun,age_flags(11)
...
!   finish = nint(maxval(age(:))+10*maxval(error(:)))
  ...
  allocate (n(counter),P(11,counter),Psum(11,counter),normPDF(11,counter))
