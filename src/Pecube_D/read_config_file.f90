...
module m_read_config_file
    ...
    character(*), parameter :: keyword_monte_carlo = "monte_carlo"

    ! Ocean temperature
    character(*), parameter :: keyword_use_ocean_temperature = "use_ocean_temperature:"
    character(*), parameter :: keyword_ocean_temperature_value = "ocean_temperature_value:"
    ...
    integer(4) :: mc_fixed_erosion_rate_index
    ...
    subroutine handle_parse_mode_normal(config)
        ...
        type(fixed_erosion_rate_t) :: fixed_erosion_rate
        ...
        if (check_for_keyword(keyword_output_folder)) then
            ...
            call extract_option_3(keyword_normal_mode, &
                keyword_error_iteration, keyword_monte_carlo, &
                config%pecube_run_mode)
          ...
          if (int_value > 0 .and. float_value >= 0.0) then
            fixed_erosion_rate%index = int_value
            fixed_erosion_rate%value = float_value
            config%mc_fixed_erosion_rates(mc_fixed_erosion_rate_index) = fixed_erosion_rate
            mc_fixed_erosion_rate_index = mc_fixed_erosion_rate_index + 1
            if (mc_fixed_erosion_rate_index > 100) then
                call log_message("mc_fixed_erosion_rate_index > 100")
                ...
                call log_message("last comment: " // trim(global_last_comment))
                call log_message("parse mode: " + global_parse_mode)
                call log_message("error code: 4baf49f199caf512421b4f0968064d55")
                close(global_file_unit)
                error stop 1
            end if
          else
            ...
            call log_message("index: " + int_value)
            call log_message("erosion rate: " + float_value)
            call log_message("index must be greater than zero and value must be zero or greater then zero")
            call log_message("'" // global_current_line // "'")
            call log_message("line: " + global_current_line_number)
            call log_message("last comment: " // trim(global_last_comment))
            call log_message("parse mode: " + global_parse_mode)
        ...
        else if (check_for_keyword(keyword_use_ocean_temperature)) then
            call extract_option_bool(config%use_ocean_temperature)
        else if (check_for_keyword(keyword_ocean_temperature_value)) then
            config%ocean_temperature_value = extract_float_value()
...
!        else if (check_for_keyword(keyword_)) then
    ...
    subroutine read_config_file(config_file_name, config)
        ...
        config%use_ocean_temperature = .false.
        config%ocean_temperature_value = 2.0 ! at -2 km (below sea level)
        ...
        if (allocated(config%mc_fixed_erosion_rates)) then
            deallocate(config%mc_fixed_erosion_rates)
        endif

        allocate(config%mc_fixed_erosion_rates(100))

        ! Initialize values
        do i = 1, 100
            fixed_erosion_rate%index = -1
            fixed_erosion_rate%value = -1.0
            config%mc_fixed_erosion_rates(i) = fixed_erosion_rate
        end do

        mc_fixed_erosion_rate_index = 1
