...
! WK, 2015.02.19, Monte Carlo
    ...
    if (config%mc_use_monte_carlo) then
        if (.not. allocated(mc_random_erosion_rate)) then
            ...
            do i = 1, 100
                if (config%mc_fixed_erosion_rates(i)%index > 0) then
                    mc_random_erosion_rate(config%mc_fixed_erosion_rates(i)%index) = config%mc_fixed_erosion_rates(i)%value
                    call log_message("use fixed erosion rate for index: " + config%mc_fixed_erosion_rates(i)%index + ", value: " + &
                        config%mc_fixed_erosion_rates(i)%value)
                else
                    exit
                endif
...
! WK, 2014.03.05: read in temperature file name
      ...
      do kstep=0,istep-1
          ...
          read (7, *) intrusion_start, intrusion_end, intrusion_temperature, maybe_hold_period
...
! iout indicates if the T information is to be saved at the end of the stage
      ...
      temperature_hold_period = maybe_hold_period
...
! nsurf: number of surface points
! tmsl: fixed temperature mean at sea level (z=0) (in degC)
! zsurf(i): zvalue of the given surface point above sea level (in km)
! tlapse: lapse rate (in degC/km)
! tsurf: temperature at the given surface point (in degC)

! nz: number of layers in the model in z direction
! in: current index
! zh: actual z value (height) of the given surface point (in km)
! zl: thickness of crustal layer (= depth at which t is fixed) (in km), total model thickness
! zp: z value of node inside model (in km)
! tp: temperature at the given index inside the model (in degC)
! tmax: basal temperature (at z=-zl) (in degC)
